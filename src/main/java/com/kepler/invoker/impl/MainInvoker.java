...
public class MainInvoker implements Imported, Invoker {
	...
	public Object invoke(Request request) throws Throwable {
		// 是否开启了流控
		return PropertiesUtils.profile(
				this.profile.profile(request.service()), MainInvoker.THRESHOLD_ENABLED_KEY, MainInvoker.THRESHOLD_ENABLED_DEF)
				? this.check(request) : this.uncheck(request);
	}
